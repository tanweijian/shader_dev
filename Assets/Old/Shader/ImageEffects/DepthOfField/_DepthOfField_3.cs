...
public class _DepthOfField_3 : MonoBehaviour {
    ...
    void OnRenderImage (RenderTexture src ,RenderTexture dst ) {
        ...
        focalDist = Mathf.Clamp(focalDist, GetComponent<Camera>().nearClipPlane, GetComponent<Camera>().farClipPlane);
        float fd = focalDist / (GetComponent<Camera>().farClipPlane - GetComponent<Camera>().nearClipPlane);
