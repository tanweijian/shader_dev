...
Shader "Tut/Lighting/ForwardAdd/Indicator/unity_LightPosition_2" {
	Properties {
	...
	SubShader {
		...
		vertOut vert(appdata_base v)
		{
			...
			float3 toLight = unity_LightPosition[2].xyz - viewpos.xyz * unity_LightPosition[2].w;
			float lengthSq = dot(toLight, toLight);
			float atten = 4.0 / (1.0 + lengthSq * unity_LightAtten[2].z);
